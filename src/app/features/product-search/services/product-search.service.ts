...
export class ProductSearchService {
  ...
  // Callback para notificar cuando se debe recargar la búsqueda
  ...
  private buildHttpParams(params: SearchParams): HttpParams {
    ...
    // Query de búsqueda (Query con mayúscula)
    if (params.query) {
    ...
    // Filtros específicos
    if (params.filters) {
      // BrandIds (pueden ser múltiples separados por coma)
  ...
  /**
   * Generar filtros dinámicos basados en los productos y metadata
   */
  private generateFilters(products: Product[], metadata?: BackendMetadata): FilterOption[] {
    ...
    // Usar marcas de metadata si están disponibles, sino extraer de productos
    if (
